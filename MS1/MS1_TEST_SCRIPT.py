...
# **Test Script**
test_df = pd.read_csv(r"C:\Users\yousef\Downloads\SongPopularity (1).csv")
pd.set_option('display.max_columns', None)
test_df.head()
orig_df = pd.read_csv(r"C:\Users\yousef\Downloads\SongPopularity (1).csv")
...
test_target = test_df['Popularity']
...
test_target_scaled = y_scaler.transform(test_target_df.values.reshape(-1, 1))

test_features_scaled_df = pd.DataFrame(test_features_scaled, columns=test_features_df.columns)
test_target_scaled_df = pd.DataFrame(test_target_scaled, columns=['Scaled Popularity'])
test_features_scaled_df
test_target_scaled_df
...
test_target = test_target_scaled_df.copy()
test_features.head()
test_target
...
mse_loaded = mean_squared_error(test_target, y_pred)
RMSE_loaded = mean_squared_error(test_target, y_pred, squared=False)
MAE_loaded = mean_absolute_error(test_target, y_pred)
r2_loaded = r2_score(test_target, y_pred)

print("Loaded XGBoost regression model:")
print("Mean squared error:", mse_loaded)
print("Root Mean Squared Error:", RMSE_loaded)
print("Mean Absolute Error:", MAE_loaded)
print("R-squared score:", r2_loaded)